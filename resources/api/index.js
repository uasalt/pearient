...
var url = process.env.wsURL || 'wss://ws.elemsocial.com/user_api'
var socket = null
...
export function setUnathorized(cb) {
    unauthorized = cb
}
    ...
    socket.onmessage = async function (event) {
        ...
        if (rsaPublicServer) {
            if (aesServerKey) {
                ...
                if (decryptedData.status == 'error' && (
                    decryptedData.message == 'account_not_found' ||
                    decryptedData.message == 's_key_invalid'
                ) && unauthorized) {
                    unauthorized(decryptedData)
                }
